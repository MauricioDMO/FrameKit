...
async function expectProjectFiles (
  ...
  expect(dockerfile).toContain('ENV FRAMEKIT_DATABASE_PATH=/data/framekit.sqlite')
  expect(dockerfile).toContain('RUN mkdir -p /data && chown node:node /data')
  expect(dockerfile.indexOf('RUN mkdir -p /data && chown node:node /data')).toBeLessThan(dockerfile.indexOf('USER node'))
  ...
  expect(dockerignore).toContain('.framekit-data/')
