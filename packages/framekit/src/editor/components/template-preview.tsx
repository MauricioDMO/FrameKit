...
export function TemplatePreview({ width, height, label, actualSizeLabel, fitToViewLabel, children }: TemplatePreviewProps) {
  ...
  useEffect(() => {
    ...
    function zoomAtPointer(event: WheelEvent) {
      ...
      setView((current) => {
        const scale = Math.min(Math.max(current.scale * Math.exp(-event.deltaY * 0.002), MIN_SCALE), MAX_SCALE)
        // Keep the artwork point beneath the pointer fixed as the scale changes.
        return { scale, x: pointerX - ((pointerX - current.x) / current.scale) * scale, y: pointerY - ((pointerY - current.y) / current.scale) * scale }
      })
      setViewMode('custom')
    }
