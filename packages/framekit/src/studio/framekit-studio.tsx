...
import { ImageIcon, Images, Moon, PanelLeftClose, PanelLeftOpen, Settings, Sun } from 'lucide-react'
  ...
  return (
    <div className={`min-h-screen bg-[#f0eee7] lg:grid ${sidebarCollapsed ? 'lg:grid-cols-[56px_1fr]' : 'lg:grid-cols-[296px_1fr]'} xl:h-dvh xl:min-h-0 xl:overflow-hidden dark:bg-[#17221d]`}>
      <aside className={`flex flex-col border-b border-white/10 bg-[#10271f] text-white lg:sticky lg:top-0 lg:h-screen lg:border-r lg:border-b-0 ${sidebarCollapsed ? 'h-20.5 lg:h-screen' : ''}`}>
        {sidebarCollapsed ? (
          <>
            <button type="button" onClick={toggleSidebar} aria-label={messages.sidebar.expandLabel} title={messages.sidebar.expandLabel} className="inline-flex h-20.5 w-full shrink-0 items-center justify-center border-b border-white/10 text-[#c8f7d9] transition hover:bg-white/8 focus:ring-2 focus:ring-inset focus:ring-[#c8f7d9] focus:outline-none"><span className="flex size-11 items-center justify-center rounded-xl border border-white/20 bg-white/10"><PanelLeftOpen size={18} /></span></button>
          ...
          <>
            <header className="flex h-20.5 shrink-0 items-center gap-3 border-b border-white/10 px-5">
              ...
              <button type="button" onClick={toggleSidebar} aria-label={messages.sidebar.collapseLabel} title={messages.sidebar.collapseLabel} className="ml-auto inline-flex size-10 shrink-0 items-center justify-center rounded-lg text-[#c8f7d9] transition hover:bg-white/10 focus:ring-2 focus:ring-[#c8f7d9] focus:outline-none"><PanelLeftClose size={18} /></button>
            </header>
...
function EmptyState({ isBrand }: { isBrand: boolean }) {
  ...
  return <div className="flex min-h-[60vh] items-center justify-center p-8 lg:min-h-screen"><div className="max-w-md text-center"><div className="mx-auto flex size-16 items-center justify-center rounded-2xl bg-[#173d31] text-[#b9f8d2] shadow-[0_10px_30px_rgba(23,61,49,0.25)]"><ImageIcon size={28} strokeWidth={1.7} /></div><p className="mt-7 text-xs font-bold tracking-[0.24em] text-[#577066] uppercase dark:text-[#a4b8ac]">{isBrand ? messages.brand.componentLabel : messages.emptyState.ready}</p><h1 className="mt-3 text-3xl font-black tracking-[-0.04em] text-[#17221d] dark:text-[#e6eee9]">{title}</h1><p className="mt-3 leading-7 text-[#657168] dark:text-[#b8c8be]">{description}</p></div></div>
}
