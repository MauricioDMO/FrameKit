...
- An incomplete number input draft stays local to the number control and never enters committed editor data. Preview/render use only resolved committed typed data, not a global last-valid-preview cache. Export and copy validate finite numbers, declared bounds, and `step` (default `1`); required text, color, and image values reject empty strings, while those fields accept empty values with `required: false`. Choices require declared option strings and booleans require real booleans.
