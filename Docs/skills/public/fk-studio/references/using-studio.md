...
- Field definitions select multiline text, number, choice, boolean, color, or image controls. Text values preserve newlines and may declare length limits. Choice values follow the declared ordered options and invalid values return `invalid_choice`; boolean values use a native checkbox and invalid string substitutes return `invalid_boolean`. Required values are checked on export; optional empty values are valid.
