...
3. State the cause, smallest fix, and limitation. Remember that edits persist only as typed string/finite-number/boolean data under `framekit:<slug>:v2`; there is no v1 migration. Incomplete number drafts stay inside the control, while preview/export/copy use committed resolved data.
