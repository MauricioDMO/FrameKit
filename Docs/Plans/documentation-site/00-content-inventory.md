...
| integradores/usuarios | referencia image render | `en/users/reference/http-api/image-render.md` | `packages/framekit/src/server/image-handler/**`; `e2e/image-api.spec.ts:3-63` | `migrate` |
...
| operadores | troubleshooting de deployment | `en/users/troubleshooting/deployment.md` | `packages/create-framekit/template/Dockerfile`; `tooling/smoke-docker.mjs`; `Docs/en/reference/cli.md:126-180` | `combine` |
...
| contribuidores | E2E y smoke | `en/contributors/testing/e2e-and-smoke.md` | `e2e/image-api.spec.ts`; `tooling/smoke-tarballs.mjs`; `tooling/smoke-docker.mjs` | `migrate` |
| contribuidores | CI | `en/contributors/testing/ci.md` | `.github/workflows/ci.yml:1-105`; `tooling/check-runtime-contract.mjs:60-73` | `migrate` |
...
| contribuidores | exports de paquetes | `en/contributors/distribution/package-exports.md` | package manifests; `packages/framekit/src/*.ts`; `tooling/check-runtime-contract.mjs` | `migrate` |
| contribuidores | consumer generado | `en/contributors/distribution/generated-consumer.md` | `packages/create-framekit/template/**`; `tooling/smoke-tarballs.mjs` | `migrate` |
...
| Testing, CI y gates | Los tests runtime de Vitest viven bajo el `__tests__/` más cercano; los contratos de compilación positivos y negativos viven solo en `packages/framekit/type-tests/`, agrupados por contexto; E2E usa Chromium desde `e2e/` para Studio y la API autenticada. Desde la raíz, `check:runtime` verifica manifests, docs críticas y la matriz CI; `lint`, `test`, `typecheck` y `build` son scripts recursivos; `test:e2e` ejecuta Playwright; `smoke:tarballs`, `smoke:docker` y `sync:skills` son gates/herramientas separadas. El tooling de mantenimiento vive en `tooling/` y el tooling específico permanece en su paquete. CI verifica Node `22.13.0` y `24` en Ubuntu con pnpm `11.14.0`, un smoke de consumer en Windows/Node `22.13.0` y E2E Chromium en Ubuntu/Node `22.13.0`; también ejecuta pack dry-run. No se debe prometer cobertura visual, matriz completa de browsers, clipboard como gate, ni un smoke Docker como prueba unitaria. | `AGENTS.md:24-30`; `package.json:9-20`; `playwright.config.ts:3-5`; `.github/workflows/ci.yml:1-105`; `tooling/check-runtime-contract.mjs:6-25,60-73`; `packages/framekit/package.json:82-91`; `packages/framekit/type-tests/README.md:1-11`; `packages/create-framekit/package.json:46-53`; `tooling/README.md:1-11`; `Docs/en/development/testing-and-distribution.md:1-19,58-92,192-217` |
...
| API y E2E | `packages/framekit/src/server/api-handler.ts:7-24`; `packages/framekit/src/server/image-handler/index.ts:16-80`; `packages/framekit/src/server/access/api-tokens.ts:89-165`; `e2e/image-api.spec.ts:3-63` |
| Cobertura y distribución | `packages/framekit/src/core/__tests__/fields.test.ts:4-154`; `packages/framekit/type-tests/README.md:1-11`; `packages/framekit/src/tooling/cli/__tests__/cli.test.ts:93-193`; `packages/create-framekit/src/__tests__/cli.test.ts:25-90`; `tooling/smoke-tarballs.mjs`; `tooling/smoke-docker.mjs`; `Docs/en/development/testing-and-distribution.md:1-19,58-92,129-217,323-432` |
