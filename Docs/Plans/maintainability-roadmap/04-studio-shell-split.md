...
| `FrameKitNavigationTree` and `manifestToNavigation` | Existing `editor/navigation/framekit-navigation.tsx` and `editor/navigation/navigation.ts` | Reuse without moving or duplicating them. The shell receives the computed navigation and renders the existing tree. |
