...
| --- | --- | --- |
| `PORT` | Configuración confiable del proceso usada para inferir el origen privado de loopback. | `parseImageRenderConfig()` usa `3000` por defecto y deriva `http://localhost:${PORT}` para la URL privada `/framekit/render/:id` y el contexto de render del navegador. |
...
proceso. `PORT` tiene `3000` por defecto. Chromium usa este origen privado de
loopback y no puede acceder a una red externa arbitraria.
