...
- The server-only `@mauriciodmo/framekit/server` facade with the implemented Steps 1-5 contracts, image-input preparation, PNG rendering, temporary render jobs, and the private render-page handoff.

## Known limitations

- Beta software: APIs and generated project details may change between releases.
- Studio export currently supports PNG only in the browser. The package also provides server-side PNG rendering through a private render-job/page handoff, but not public image API routes, GIF/video export, alternate image formats, scale, or DPI controls.
