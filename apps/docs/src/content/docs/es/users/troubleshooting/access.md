...
:::note
Estas rutas solo existen cuando `FRAMEKIT_AUTH_ENABLED=true`. En modo abierto,
`/editor` y `/brand` no requieren sesión, `/login` redirige a `/editor`, y
`/settings` y la API de acceso responden como no encontrados. No configures
credenciales ni SQLite para solucionar ese comportamiento.
:::

## El inicio de sesión autenticado redirige de nuevo a la página de inicio de sesión

**Síntoma:** Con `FRAMEKIT_AUTH_ENABLED=true`, abrir `/editor`, `/brand` o `/settings` redirige a `/login`, o el formulario de inicio de sesión indica que las credenciales no son válidas.

**Causa probable:** Falta la cookie de sesión, ha caducado, no es válida o pertenece a un usuario inactivo; el usuario o la contraseña enviados también pueden ser incorrectos. Si la variable de autenticación falta o es `false`, la redirección no es esperada: `/login` debe redirigir a `/editor`.
...
## El primer inicio de sesión autenticado falla en una base de datos nueva

**Síntoma:** Con `FRAMEKIT_AUTH_ENABLED=true`, el primer inicio de sesión devuelve un error de servicio no disponible en una base de datos vacía.

**Causa probable:** Falta `FRAMEKIT_ADMIN_PASSWORD` o no es válida, o `FRAMEKIT_ADMIN_USERNAME` no cumple las reglas de las cuentas. Estas variables no se usan en modo abierto.
...
**Solución:** Define `FRAMEKIT_AUTH_ENABLED=true` y valores válidos en el entorno de ejecución, y vuelve a intentar el primer inicio de sesión. Estos valores solo crean el primer administrador; cambiarlos más adelante no reemplaza a un usuario existente. Consulta [Configuración](/es/users/reference/configuration).
...
**Causa probable:** Con la autenticación activada, la sesión actual se invalida al cerrar sesión; todas las sesiones se invalidan al cambiar la contraseña, restablecerla un administrador o desactivar o eliminar la cuenta. Las sesiones también caducan después de 30 días.
...
**Causa probable:** Con la autenticación activada, solo un administrador activo puede listar y gestionar usuarios, cambiar el rol o el estado activo de otro usuario, restablecer la contraseña de otro usuario, eliminar un usuario o consultar los metadatos de los tokens de otro usuario y revocar esos tokens.
...
**Síntoma:** Con la autenticación activada, una solicitud del lado del servidor que usa `Authorization: Bearer <token>` devuelve un error de no autorizado.

**Causa probable:** El token se revocó, su propietario está inactivo o la solicitud usa un secreto truncado. El token completo solo se muestra cuando se crea. En modo abierto no se necesita un token.
