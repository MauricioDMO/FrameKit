...
Para los valores de los campos y las fuentes de imagen, consulta [Entradas de imagen](/es/users/reference/http-api/image-inputs). Consulta [Seguridad y proxies inversos](/es/users/deployment/security-and-reverse-proxies) para conocer las restricciones de despliegue y [Errores de la API de imágenes](/es/users/reference/http-api/errors) para los códigos de error.
